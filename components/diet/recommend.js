...
const Recommend = ({gender, status:{age, height, weight, disease, allergy}}) => {
  ...
  const onClickSubmitButton = (e) => { // 입력버튼 누르면
    ...
    if(e.target.id === 'button1'){
      axios.post("/api/chat", { prompt: res }).then((res) => { // prompt 변수에 res값을 담아서 포스트요청(api/chat.ts로) / then에서 응답 받은 값을(res)파라미터에 할당
    ...
    <CommonLayout>
      <div className="flex w-full justify-center items-center flex-col">
        ...
        <input
          ...
          onChange={handleChange}
        />
        <button id = "button1" className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg focus:outline-none focus:shadow-outline" onClick={onClickSubmitButton}>
          입력
        </button>
      </div>
