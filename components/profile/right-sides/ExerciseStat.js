...
const ExerciseStat = () => {
  ...
  return (
    <div className='flex flex w-full h-full bg-gray-400 p-5 flex-col overflow-auto'>
      <div className='flex flex-col w-[31%]'>
      <div className='flex flex-row'>
      <button className={`flex w-1/4 h-full bg-gray-200 rounded-tl-lg rounded-tr-lg p-5 flex-col hover:bg-hover hover:transition
      ${show === 'year' ? 'bg-blue-500' : ''}`}
      onClick = {(e) => setShow("year")}>연간</button>
      <button className={`flex w-1/4 h-full bg-gray-200 rounded-tl-lg rounded-tr-lg p-5 flex-col hover:bg-hover hover:transition
      ${show === 'month' ? 'bg-blue-500' : ''}`}
      onClick = {(e) => setShow("month")}>월간</button>
      <button className={`flex w-1/4 h-full bg-gray-200 rounded-tl-lg rounded-tr-lg p-5 flex-col hover:bg-hover hover:transition
      ${show === 'week' ? 'bg-blue-500' : ''}`}
      onClick = {(e) => setShow("week")}>주간</button>
      <button className={`flex w-1/4 h-full bg-gray-200 rounded-tl-lg rounded-tr-lg p-5 flex-col hover:bg-hover hover:transition
      ${show === 'day' ? 'bg-blue-500' : ''}`}
      onClick = {(e) => setShow("day")}>일간</button>
      </div>
      {show === "year" &&   (
      <Calendar
        value={date}
        onChange={(e) => {calendarHandler(e, show, date)}}
        maxDetail='decade'
    ...
    <div
      className={`flex items-center justify-between px-4 py-2 border border-gray-300 bg-white cursor-pointer`}
    ...
    {isOpen && (
      <div className="absolute top-full w-full border border-gray-300">
      ...
      <Tooltip
        labelFormatter={(label) => {
          ...
          if (data) {
            ...
            let formattedDate;
            if (show === "day")
              formattedDate = date.toLocaleString([], { dateStyle: 'short', timeStyle: 'short' });
            else
              formattedDate = date.toLocaleString([], { dateStyle: 'short' });
  ...
  <label className='flex w-full text-gray-500 text-sm mt-1 text-center'>탄수화물(g) * 4 + 단백질(g) * 4 + 지방(g) * 9 값입니다.</label>
  </div>
    </div>
  )
}
