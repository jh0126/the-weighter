...
const DetailBoard = ({ data, comment, setComment }) => {
  ...
  return (
    <div className='flex flex-col items-end justify-center w-[80%] my-10 bg-white border-2 border-neutral-400'>
      <div className='flex items-center justify-between w-full border-b-2 h-14 border-neutral-400'>
        <div className='flex w-[10%] h-full justify-center items-center border-neutral-400 bg-neutral-300 border-r'>글번호</div>
        <div className='flex w-[5%] h-full items-center p-5 justify-center border-neutral-400  border-r'>{data.id}</div>
        <div className='flex w-[85%] h-full items-center p-5 justify-end select-none'>
          <button className='flex items-center justify-center w-24 text-white bg-red-500 rounded-lg h-14' onClick={() => onClickDeleteButton()}>
            삭제
          </button>
          <button className='flex items-center justify-center w-24 ml-5 text-white bg-green-500 rounded-lg h-14' onClick={() => onClickModifyButton()}>
            수정
          </button>
        </div>
      </div>
      <div className='flex items-center justify-between w-full border-b-2 h-14 border-neutral-400'>
        ...
        <div className='flex w-[75%] h-full px-5 items-center'>{data.title}</div>
      </div>
      <div className='flex items-center justify-between w-full border-b-2 h-14 border-neutral-400'>
        <div className='flex w-[10%] h-full justify-center items-center border-neutral-400 bg-neutral-300 border-r'>작성자</div>
      ...
      <div className='flex flex-col items-center justify-center w-full p-3 select-none'>
        <button className='flex flex-col items-center justify-center w-24 h-24 text-white rounded-full bg-button' onClick={() => onClickHitButton()}>
      ...
      <div className='flex w-full min-h-[150px] justify-center items-center border-neutral-400 border-b flex-col'>
        <div className='flex items-center justify-center w-full h-12 border-b select-none bg-neutral-300 border-neutral-400'>댓글</div>
        {comment.length >= 1 &&
          comment.map((value) => {
            return (
              <div className='flex items-center justify-between w-full border-b-2 h-14 border-neutral-400' key={value.id}>
        ...
        <div className='flex w-full min-h-[100px] justify-between pl-5 p-2 select-none'>
          ...
          <div className='flex w-[10%] border-neutral-400 border-l ml-2 justify-center items-center'>
            <button className='flex items-center justify-center w-4/5 h-20 text-white bg-button' onClick={() => onClickWriteComment()}>
              작성
            </button>
          </div>
        </div>
