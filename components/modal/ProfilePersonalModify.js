...
const ProfilePersonalModify = ({ open, onClose, name, gender, email }) => {
  ...
  const onClickSubmitButton = () => {
    if (nameValue && isConfirmEmail && isSendEmail) {
      request()
        ...
        .catch((err) => {console.error(err); alert(err.response.status)});
    } else {
  ...
  const onClickSendEmailButton = () => {
    ...
    return (
      ...
      <Box className="absolute flex w-2/6 h-[70%] bg-white rounded-2xl items-center justify-center shadow-shadow mr-20 text-black top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
        <div className='flex w-[90%] h-full flex-col items-center justify-center'>
          <div className='flex mb-32'>{"이미지"}</div>
          <label>이름</label>
          <input className="flex w-2/5 max-w-[200%] border-gray-400 border-[1px] rounded-lg px-3" 
            type="name" 
            value={nameValue} 
            onChange={setNameHandler}/>
          <label>성별</label>
          <ul className="flex w-3/5 justify-center text-sm font-medium text-gray-900 border border-gray-200 rounded-lg sm:flex dark:border-gray-600 dark:text-white">
      <li className="w-full border-b border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600">
        <div className="flex items-center pl-3 dark:bg-neutral-900">
          <input
            id="checkbox-male"
            type="radio"
            name="gender"
      ...
      <li className="w-full border-b border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600">
        <div className="flex items-center pl-3 dark:bg-neutral-900">
          ...
          <label
            ...
            className="w-full py-3 ml-2 text-sm font-medium text-gray-900 dark:text-gray-300 rounded-r-lg"
          >
            여성
          </label>
        </div>
          ...
          <input
            className="flex w-full max-w-[200%] h-12 border-gray-400 border-[1px] rounded-lg px-3"
            type="email" 
            value={emailValue} 
        ...
        {isConfirmEmail && isSendEmail ? (
          <div className="flex w-3/5 justify-end items-center mt-2">
            <label className="flex text-blue-500 text-sm mr-2">이메일이 확인되었습니다.</label>{" "}
          </div>
        ) : isConfirmEmail ? (
          <div className="flex w-3/5 justify-end items-center mt-2">
            <label className="flex text-red-500 text-sm mr-2">이메일을 인증해주세요.</label>
            <button className="flex w-2/5 h-12 bg-button rounded-lg justify-center items-center text-white" onClick={() => onClickSendEmailButton()}>
              인증메일 전송
            </button>
          </div>
        ) : (
